...
import { Filter, Search, PackageOpen, Loader2, Eye, ShoppingCart, ExternalLink } from "lucide-react";
...
const categoryLabels: Record<ProductCategory, string> = {
  gadgets: "Gadgets", telco: "Telco", fintech: "Fintech",
  events: "Events", fashion: "Fashion", courses: "Courses",
};
...
const typeLabels: Record<string, string> = { physical: "Physical", digital: "Digital", lead: "Sign-Up" };
const typeBadgeColors: Record<string, string> = {
  physical: "", digital: "bg-blue-500/10 text-blue-600 border-blue-500/20",
...
const PublicMarketplace = () => {
  ...
  const categoryCounts = products.reduce((acc, p) => { acc[p.category] = (acc[p.category] || 0) + 1; return acc; }, {} as Record<string, number>);
  const typeCounts = products.reduce((acc, p) => { acc[p.productType] = (acc[p.productType] || 0) + 1; return acc; }, {} as Record<string, number>);
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      <div className="flex items-baseline gap-3">
        <h1 className="text-2xl md:text-3xl font-bold font-display">Products</h1>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto">
            <Button variant="ghost" size="sm" onClick={() => setSortBy(sortBy === "price" ? "name" : "price")} className="text-muted-foreground h-8 text-xs">
              <Filter className="h-3 w-3 mr-1" /> Sort: {sortBy === "price" ? "Price" : "Name"}
            </Button>
          </div>
        </div>
      </div>
                ...
                <Card className="border-border/50 group hover:border-primary/30 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer overflow-hidden h-full">
                  <div className="relative overflow-hidden">
                    <AspectRatio ratio={4 / 3}>
                      {product.assets?.images?.length > 0 ? (
                        <img src={product.assets.images[0]} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                      ) : (
                        <div className="w-full h-full bg-muted flex items-center justify-center"><span className="text-5xl">{product.image}</span></div>
                      )}
                    </AspectRatio>
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                      <div className="bg-background/90 backdrop-blur-sm rounded-full p-2.5 shadow-lg"><Eye className="h-4 w-4 text-foreground" /></div>
                    </div>
                  ...
                  <CardContent className="p-4 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between gap-1.5 pt-1">
                      <p className="text-sm font-bold text-foreground">{product.price > 0 ? formatNaira(product.price) : "Free"}</p>
                      {isLead ? (
                        <Button size="sm" className="volt-gradient text-xs h-8 px-3 shadow-md" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
                          <ExternalLink className="h-3 w-3 mr-1" /> Sign Up
                        </Button>
                      ) : (
                        <Button size="sm" onClick={(e) => {
                          e.preventDefault(); e.stopPropagation();
                          addItem({ productId: product.id, name: product.name, slug: product.slug, image: product.image, imageUrl: product.assets?.images?.[0], price: product.price, commissionRate: product.commissionRate });
                          toast.success(`${product.name} added to cart!`);
                        }} className="volt-gradient text-xs h-8 px-3 shadow-md">
