...
export function ManualSaleDialog({ open, onOpenChange }: ManualSaleDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload proof file (not required for leads)
      if (proofFile) {
      ...
      // Insert transaction (not for leads until verified)
      if (!isLead) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
              ...
              <SelectContent>
                {products.map((p) => (
                  <SelectItem key={p.id} value={p.id}>
                    {p.image} {p.name} {p.productType !== "physical" ? `(${p.productType})` : ""}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {/* Lead-specific: conversion status */}
          {isLead ? (
            <div className="space-y-1.5">
